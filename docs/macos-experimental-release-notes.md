...
**macOS Universal DMG**: [AutoLooper.0.4.3.mac.dmg](https://github.com/YoshimiKudo/AutoLooper/releases/download/v0.4.3-macos-experimental/AutoLooper.0.4.3.mac.dmg)

**macOS Universal ZIP**: [AutoLooper.0.4.3.mac.zip](https://github.com/YoshimiKudo/AutoLooper/releases/download/v0.4.3-macos-experimental/AutoLooper.0.4.3.mac.zip)

**Checksum**: [SHA256SUMS-mac.txt](https://github.com/YoshimiKudo/AutoLooper/releases/download/v0.4.3-macos-experimental/SHA256SUMS-mac.txt)
...
## Changes in v0.4.3 / v0.4.3 の変更点

- Fixed settings being reset to defaults on every launch.
- Language, display unit, detection settings, custom detection preset, output folder, and filename suffix are now saved to the app preferences file.
- AutoLooper does not use microphone recording, and microphone/camera permissions remain denied.

- 起動するたびに設定が初期値へ戻る問題を修正しました。
- 言語、表示単位、検出設定、Custom検出プリセット、保存先、ファイル名追加文字をアプリ設定ファイルへ保存するようにしました。
- AutoLooperはマイク録音機能を使いません。マイク/カメラ権限は引き続き許可しません。
